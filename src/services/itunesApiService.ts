...
class iTunesAPIService {
  ...
  private cleanSearchTerm(term: string): string {
    const clean = term
      .toLowerCase()
      // Remove common suffixes and prefixes that interfere with search
      ...
      .replace(/official video/g, '')
      .replace(/official audio/g, '')
      .replace(/[^\w\sà-úÀ-ÚçÇ]/g, ' ') // Replace special chars with space instead of removing
      .replace(/\s+/g, ' ') 
      .trim();
    return clean;
  }
  ...
  /**
   * More robust search that handles common metadata issues
   */
  async searchWithRetry(artist: string, title: string): Promise<string | null> {
    // 1. Try exact search (cleaned)
    let artwork = await this.searchTrack(artist, title);
    if (artwork) return artwork;

    // 2. If failed, try searching only by title if it looks like it contains the artist
    if (title.toLowerCase().includes(artist.toLowerCase())) {
        artwork = await this.searchTrack('', title);
        if (artwork) return artwork;
    }

    // 3. Try searching just the title if it's long enough
    if (title.length > 5) {
        artwork = await this.searchTrack('', title);
        if (artwork) return artwork;
    }
  ...
  private findBestMatch(results: iTunesTrack[], artist: string, track: string): iTunesTrack | null {
    ...
    const scoredResults = results.map(result => {
      const resultArtistOrig = result.artistName.toLowerCase();
      const resultTrackOrig = result.trackName.toLowerCase();
      const resultArtistNorm = normalize(result.artistName);
      const resultTrackNorm = normalize(result.trackName);
      
      let score = 0;
      
      // Artist Match Score
      if (resultArtistNorm === normArtist) {
        score += 25; // Exact match (normalized)
      } else if (resultArtistNorm.includes(normArtist) || normArtist.includes(resultArtistNorm)) {
        score += 15; // Partial match or collaboration
      } else if (resultArtistOrig.includes(artist.toLowerCase()) || artist.toLowerCase().includes(resultArtistOrig)) {
        score += 10; // Simple lowercase match
      }
      
      // Track Match Score
      if (resultTrackNorm === normTrack) {
        score += 20; // Exact match
      } else if (resultTrackNorm.includes(normTrack) || normTrack.includes(resultTrackNorm)) {
        score += 10; // Partial match
      }
      
      // If we find the exactly requested strings in the result (case insensitive)
      if (result.artistName.toLowerCase().indexOf(artist.toLowerCase()) !== -1) score += 5;
      if (result.trackName.toLowerCase().indexOf(track.toLowerCase()) !== -1) score += 5;
    ...
    console.log(`Best match score: ${best?.score} for ${best?.result.trackName} by ${best?.result.artistName}`);
    
    return best && best.score >= 15 ? best.result : results[0]; // If no good match, fallback to first result if it exists
  }
